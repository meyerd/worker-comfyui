...
if [ ! -d "$CUSTOM_NODES_DIR/ComfyUI-WanVideoWrapper" ]; then
    ...
    cd $CUSTOM_NODES_DIR/ComfyUI-WanVideoWrapper
    git pull
fi
if [ ! -d "$CUSTOM_NODES_DIR/ComfyUI-KJNodes" ]; then
    ...
    cd $CUSTOM_NODES_DIR/ComfyUI-KJNodes
    git pull
fi


echo "🔧 Installing KJNodes packages..."
uv pip install --no-cache-dir -r $CUSTOM_NODES_DIR/ComfyUI-KJNodes/requirements.txt &
KJ_PID=$!

echo "🔧 Installing WanVideoWrapper packages..."
uv pip install --no-cache-dir -r $CUSTOM_NODES_DIR/ComfyUI-WanVideoWrapper/requirements.txt &
WAN_PID=$!
...
if [ "$download_wan_fun_and_sdxl_helper" == "true" ]; then
  ...
  UNION_DIR="$COMFYUI_DIR/models/controlnet/SDXL/controlnet-union-sdxl-1.0"
  mkdir -p "$UNION_DIR"
...
if [ ! -f "$COMFYUI_DIR/models/upscale_models/4xLSDIR.pth" ]; then
    if [ -f "/4xLSDIR.pth" ]; then
        mv "/4xLSDIR.pth" "$COMFYUI_DIR/models/upscale_models/4xLSDIR.pth"
